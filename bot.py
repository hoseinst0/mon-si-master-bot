...
from config import BOT_TOKEN, WEB_PANEL_ENABLED, WEB_PANEL_PORT
...
async def run_web_panel():
    """پنل وب مدیریت رو به‌صورت هم‌زمان با ربات، روی همون پروسس بالا میاره."""
    import uvicorn
    from web_panel.app import app as web_app

    uv_config = uvicorn.Config(web_app, host="0.0.0.0", port=WEB_PANEL_PORT, log_level="info")
...
async def main():
    ...
    dp.include_router(handlers_deal.router)
    ...
    dp.include_router(handlers_admin_panel.router)
    ...
    if WEB_PANEL_ENABLED:
        ...
        logging.info("پنل وب روی پورت %s فعال شد", WEB_PANEL_PORT)
