...
def roll_keyboard(qid: str, step_idx: int) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
...
def reroll_choice_keyboard(qid: str, step_idx: int) -> InlineKeyboardMarkup:
    """فقط بعد از یک نتیجه‌ی 'شکست' نشان داده می‌شود، وقتی بازیکن سنگ خلسه دارد."""
    kb = InlineKeyboardBuilder()
    kb.button(text="✅ همین نتیجه رو قبول می‌کنم", callback_data=f"quest:accept_roll:{qid}:{step_idx}", style="primary")
    kb.button(text="🔮 با سنگ خلسه رول مجدد بزن", callback_data=f"quest:reroll:{qid}:{step_idx}", style="success")
    kb.adjust(1)
    return kb.as_markup()
