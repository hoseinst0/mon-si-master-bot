...
def _next_day_target_hours() -> float:
    """ساعتِ واقعیِ لازم برای رسیدن روز بعدِ آکادمی، با کمی رندوم بودن (نه دقیقاً ثابت)
    تا نامه‌ها/کوئست‌ها همیشه یک زمان مشخص و قابل‌پیش‌بینی نیایند."""
    base = config.HOURS_PER_ACADEMY_DAY
    jitter = min(base * 0.25, config.LETTER_INTERVAL_JITTER_DAYS * 24)
    return max(1.0, random.uniform(base - jitter, base + jitter))
...
async def _advance_day_and_notify(bot):
    ...
    state["next_day_target_hours"] = _next_day_target_hours()
...
async def letter_scheduler_loop(bot):
    ...
    while True:
        try:
            state = db.get_state()
            target = state.get("next_day_target_hours") or config.HOURS_PER_ACADEMY_DAY
