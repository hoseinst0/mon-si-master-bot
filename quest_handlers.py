...
async def send_step(message: Message, player: dict, q: dict, step_idx: int):
    ...
    can_reroll = player.get("items", {}).get("سنگ خلسه", 0) not in (0,)
    await message.answer(text, reply_markup=kb.roll_keyboard(q["id"], step_idx, can_reroll))
...
async def cb_roll(callback: CallbackQuery):
    ...
    await do_roll(callback, qid, step_idx, used_reroll=False)
...
async def cb_reroll(callback: CallbackQuery):
    ...
    async with db.player_lock(user_id):
        ...
        if not utils.remove_item(player, "سنگ خلسه", 1):
            ...
            return
        db.save_player(player)
    await do_roll(callback, qid, step_idx, used_reroll=True)


async def do_roll(callback: CallbackQuery, qid: str, step_idx: int, used_reroll: bool):
    q = qd.get_quest(qid)
    if not q:
    ...
    async with db.player_lock(user_id):
        player = db.get_player(user_id)
        if qid not in player.get("quests_active", {}):
            await callback.answer("این کوئست دیگر فعال نیست.", show_alert=True)
            return
