...
def roll_dice(exclude_fail: bool = False) -> int:
    """exclude_fail=True یعنی تضمینی نتیجه در بازه‌ی 'شکست' (۱ یا ۲) نمی‌افتد.
    برای رول مجدد با سنگ خلسه استفاده می‌شود تا مصرف آیتم همیشه نتیجه‌ی بهتری بدهد."""
    if exclude_fail:
