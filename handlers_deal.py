...
async def handle_deal_start(message: Message, bot: Bot, deal_id: str):
    ...
    logging.info("DEBUG handle_deal_start: start deal_id=%s user=%s", deal_id, message.from_user.id)
    deal = await db.get_deal(deal_id)
    logging.info("DEBUG handle_deal_start: deal found=%s", bool(deal))
    if not deal:
    ...
    if not deal.get("counterparty_id"):
        ...
        deal = await db.get_deal(deal_id)
        logging.info("DEBUG handle_deal_start: counterparty set, role=%s", counterparty_role)
    ...
    logging.info("DEBUG handle_deal_start: confirmation message sent OK")
