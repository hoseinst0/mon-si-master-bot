...
from config import MONGO_URI, DB_NAME, TRUST_SCORE_START, MIDDLEMAN_FEE_TIERS
...
async def is_kyc_verified(user_id: int) -> bool:
    user = await users_col.find_one({"_id": user_id})
    return bool(user and user.get("kyc_verified"))


async def mark_kyc_requested(user_id: int):
    await users_col.update_one({"_id": user_id}, {"$set": {"kyc_requested_at": time.time()}})


async def set_kyc_verified(user_id: int, verified: bool = True):
    await users_col.update_one({"_id": user_id}, {"$set": {"kyc_verified": verified}})


def calculate_middleman_fee(amount: float, is_vip: bool = False) -> dict:
    """
    نرخ حق واسط رو بر اساس جدول MIDDLEMAN_FEE_TIERS محاسبه می‌کنه.
    ردیف‌های عادی مبلغ ثابت دارن؛ آخرین ردیف (بدون سقف) درصدیه.
    خروجی: {"fee": مبلغ کارمزد, "tier_label": توضیح بازه, "total": قیمت + کارمزد}
    """
    for tier in MIDDLEMAN_FEE_TIERS:
        lower = tier["min"]
        upper = tier["max"]
        if amount >= lower and (upper is None or amount < upper):
            if "fee_percent" in tier:
                percent = tier["vip_fee_percent"] if is_vip else tier["fee_percent"]
                fee = amount * percent / 100
            else:
                fee = tier["vip_fee"] if is_vip else tier["fee"]
            upper_label = f"{upper:,}" if upper is not None else "به بالا"
            tier_label = f"{lower:,} - {upper_label}"
            return {"fee": fee, "tier_label": tier_label, "total": amount + fee}

    # اگه به هر دلیلی هیچ ردیفی مچ نشد (مثلاً مبلغ منفی)، از ردیف اول استفاده کن
    fallback = MIDDLEMAN_FEE_TIERS[0]
    fee = fallback["vip_fee"] if is_vip else fallback["fee"]
    return {"fee": fee, "tier_label": "پیش‌فرض", "total": amount + fee}
...
async def create_deal(initiator_id: int, initiator_username: str, initiator_role: str,
    ...
    doc.update(extra)  # مثلاً fee_amount / fee_tier_label / is_vip_fee که از جدول نرخ حق واسط میاد
...
async def set_deal_counterparty(deal_id: str, counterparty_id: int,
                                 counterparty_username: str, counterparty_role: str):
...
async def set_deal_confirmed(deal_id: str, side: str):
    field = "counterparty_confirmed" if side == "counterparty" else "initiator_confirmed"
    await deals_col.update_one({"_id": deal_id}, {"$set": {field: True}})


async def find_active_deal_for_user(user_id: int):
    return await deals_col.find_one({
        "$or": [{"initiator_id": user_id}, {"counterparty_id": user_id}],
        "status": {"$in": ["awaiting_proof", "awaiting_admin"]},
    })


async def set_deal_proof(deal_id: str, side: str, file_id: str, detected_amount: float = None):
    field = "counterparty_proof" if side == "counterparty" else "initiator_proof"
    update = {field: file_id, "status": "awaiting_admin"}
    if detected_amount is not None:
        amount_field = "counterparty_proof_detected_amount" if side == "counterparty" else "initiator_proof_detected_amount"
        update[amount_field] = detected_amount
    await deals_col.update_one({"_id": deal_id}, {"$set": update})


async def set_deal_admin_approval(deal_id: str, side: str, approved: bool):
    field = "counterparty_admin_approved" if side == "counterparty" else "initiator_admin_approved"
...
async def set_deal_group(deal_id: str, chat_id: int, invite_link: str):
    await deals_col.update_one(
        {"_id": deal_id},
        {"$set": {"group_chat_id": chat_id, "group_invite_link": invite_link}},
    )
